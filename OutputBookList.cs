            ...
            using (StreamWriter sw = new StreamWriter(this.filePath, false))
            {
                foreach (NodeBookMap map in result)
                {
                    ...
                    foreach (int index in this.outputRange)
                    {
                        switch (index)
                        {
                            case OutputBookIndex.ISBN:
                                sw.WriteLine(Constants.TABINDENT + "ISBN: " + map.NodeBook.ISBN);
                                break;
                            ...
                            case OutputBookIndex.DATE:
                                ...
                                break;
                            case OutputBookIndex.PRICE:
                                sw.WriteLine(Constants.TABINDENT + "价钱: " + map.NodeBook.Price);
                                break;
                            case OutputBookIndex.PRESS:
                                sw.WriteLine(Constants.TABINDENT + "出版社: " + map.NodeBook.Press);
                                break;
